...
from .catalog import MODELS, build_tasks, split_tasks
...
def cmd_bench(args) -> int:
    ...
    print(f"\n{measured} measured tasks after {args.warmup} warm-up · "
          f"ceiling ${args.ceiling:.2f} · models "
          f"{'/'.join(MODELS)}\n")
    print(_table(["configuration", "total", "p50 exec", "p99 exec", "p99/p50",
                  "solve rate", "$/solved", "cache", "aborted"], rows))
    ...
    print("\np50/p99 are over executed tasks; cached and refused tasks cost nothing "
          "and would drag the median to zero.\n")
    ...
    if args.json:
        payload = {r.config: {
            ...
            "solve_rate": r.solve_rate, "cost_per_solved": r.cost_per_solved,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bench = sub.add_parser("bench", help="ablation across the four controls")
    bench.add_argument("--tasks", type=int, default=400)
    bench.add_argument("--warmup", type=int, default=120)
    bench.add_argument("--ceiling", type=float, default=0.15)
    bench.add_argument("--json", help="write raw results here")
    bench.set_defaults(func=cmd_bench)

    sweep = sub.add_parser("sweep", help="cost against solve rate as the ceiling moves")
    sweep.add_argument("--tasks", type=int, default=400)
    sweep.add_argument("--warmup", type=int, default=120)
    sweep.set_defaults(func=cmd_sweep)

    explain = sub.add_parser("explain", help="trace the decisions for one task")
